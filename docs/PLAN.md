...
**Build:** Ported the performance scatter and importance pie to `ui/charts.py` as pure `(result, display_metric[, selected_idx]) → go.Figure` functions; the results page embeds `fig.to_json()` and renders with vendored `plotly.min.js`. Best-configuration shown as a per-metric panel.
