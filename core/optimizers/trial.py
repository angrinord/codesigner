...
from .timing import STATUS_CRASHED, timed_evaluation
...
def evaluate_trial(model, config, X_train, y_train, X_val, y_val, metrics, seed=0):
    ...
    Returns ``(scores, run_info)``. A trial that failed — the model raised, or
    returned something unscoreable — scores 0.0 on every metric, with
    ``run_info["status"]`` set and the reason under
    ``additional_info["error"]``. That is a data point (this configuration is
    unusable), not the end of the run; whether too many failures in a row should
    stop the search is the collector's decision, not this function's.
    """
    failure = None
    y_pred = None
    ...
    with timed_evaluation(seed=seed) as run_info:
        ...
        except Exception as exc:  # noqa: BLE001 — a model may fail any way it likes
