...
an *original* metric (the one its first run used, kept only so the page can say
the two have diverged). Running again may change the primary, and every change
is confirmed first: the trials already accumulated were *chosen* under the old
objective, so the sample carries its bias forward even though the optimizer
re-reads their scores under the new one.
...
def decide_run(original_metric, primary_metric, chosen_metric):
    ...
    Returns (action, optimize_metric):
      ...
      * ("warn", chosen)   — would change the metric being optimized; confirm
                             before running.
      * ("run", primary)   — the chosen metric is already the one being
                             optimized; just run.

    Every change is confirmed, not only the first. It used to warn once, on the
    divergence from *original*, and afterwards run with the current primary no
